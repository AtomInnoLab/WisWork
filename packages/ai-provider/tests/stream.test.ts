...
describe('streamForProvider: anthropic', () => {
  ...
  it('never sends an empty assistant content array when history has edits-only replies', async () => {
    ...
    // on Anthropic (upstream#12 / #22 class of multi-turn failures).
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        okResponse(
  ...
  it('replaces an HTML error body (e.g. a gateway block page) with a readable note', async () => {
    const html =
      '<!doctype html>\n<html>\n<head><title>Upstream</title></head><body>app shell</body></html>'
...
describe('streamForProvider: 200 + non-stream JSON instead of SSE', () => {
  const creditsNotice =
    'Your upstream credits have been exhausted. Please visit https://www.provider.example/pricing to purchase more credits.'
  ...
  it('gemini route: a generic insufficient-credits notice becomes AiCreditsError', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
          candidates: [
            {
              content: {
                parts: [{ text: 'Insufficient credits; contact the configured provider.' }],
              },
            },
          ],
