...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'wiswork-save-dir-'))
})
...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'WisWork')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/WisWork without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'WisWork'))
    expect(existsSync(join(documents, 'WisWork'))).toBe(true)
  })
})
