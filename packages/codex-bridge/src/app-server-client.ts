...
const ALLOWED_NOTIFICATIONS = new Set<string>([
...
const PINNED_USER_AGENT = CODEX_CLI_VERSION.replace(' ', '/')
...
export class CodexAppServerClient {
  ...
  async initialize(): Promise<InitializeResponse> {
    ...
    try {
      ...
      if (
        !isRecord(response) ||
        response.userAgent !== PINNED_USER_AGENT ||
  ...
  #handleNotification(notification: JsonRpcNotification): void {
    if (
      !KNOWN_NOTIFICATIONS.has(notification.method) ||
      !ALLOWED_NOTIFICATIONS.has(notification.method)
    ) {
