...
The SQLite schema stores only live binding ids, Relay-computed OIDC subject hashes, public keys, hosts, origins, approved data-capability lists, and timestamps. Revocation physically deletes the binding row, and startup transactionally prunes rows marked revoked by older schema-v1 binaries, so revoked history cannot grow without bound. It does not store tokens, private keys, document content, requests, challenges, signatures, session ids, or session capabilities.
