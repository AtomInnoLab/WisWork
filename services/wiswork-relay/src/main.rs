...
async fn main() {
    ...
    let binding_database = if pairing_resume_enabled {
        match std::env::var("WISWORK_RELAY_BINDING_DB") {
            ...
            _ => panic!("WISWORK_RELAY_BINDING_DB is required when pairing resume is enabled"),
        }
    } else {
        None
    };
