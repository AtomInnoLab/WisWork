...
#### Final-family migration matrix (2026-08-28 audit)

| Tool(s)                                                                    | Legacy write path                                              | Engine/package capability                                                                                                                                                                                                          | Canonical decision                                                                                                                                                                                                         |
| -------------------------------------------------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `add_table`, `edit_table_cell`, `edit_table_structure`, `edit_table_style` | Table insertion, repeated cell writes, and structure/style IPC | Table XML support exists, but insertion fills cells through separately committed writes and structure edits can replace source ids. No canonical package postcondition proves the complete grid/text/style state as one operation. | Keep the complete table family on legacy; do not expose a partial canonical batch.                                                                                                                                         |
| `add_chart`, `edit_chart`                                                  | Chart insertion/edit IPC                                       | Chart writes can update chart parts, embedded workbooks, and style/color/user-shape relationships. The canonical host has no strict chart dependency manifest and round-trip verifier for every affected part.                     | Keep the complete chart family on legacy until package-dependency verification exists.                                                                                                                                     |
| `set_slide_background`                                                     | Background IPC, including a host-side loop for `slideIndex=-1` | `setSlideBackground` patches slide XML deterministically; `fingerprintSlide` includes the background and slide dependency state.                                                                                                   | Migrate solid backgrounds to strict `set_slide_background` operations. Bind each to durable slide id + slide fingerprint; compile `-1` into one bounded atomic transaction and reject over-limit decks before preparation. |

The background operation has no generic payload and accepts only a color plus a slide target. Tables and charts remain intentionally legacy rather than returning misleading partial receipts.
