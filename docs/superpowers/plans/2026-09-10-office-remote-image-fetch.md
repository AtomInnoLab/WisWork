...
- PC 图片链路：目标测试通过；审查补充了鉴权/刷新阶段的 deadline 与取消测试；Shell 全套 615 个通过、11 个跳过；Shell 类型检查和生产构建通过。
...
- Relay release build 通过。最终独立跨组件安全审查发现的响应期并发许可、端到端 deadline 和精确 200 状态校验均已修复并补充回归测试；修复后 Relay 31 个单元测试、54 个集成测试和严格 Clippy 再次通过。
