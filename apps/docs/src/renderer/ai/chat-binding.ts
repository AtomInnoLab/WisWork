...
export function createChatBindingCoordinator<TMessage = unknown>({
  ...
  const bind = async (nextPath: string | null): Promise<void> => {
    ...
    // First save is the same untitled document. Keep its UI/model context, but
    // suspend persistence until the store has atomically moved the chat target.
    if (
      previousPath === null &&
      nextPath !== null &&
    ...
    try {
      ...
      // Chat persistence/history is best effort. A later path change retries it.
    }
  }
