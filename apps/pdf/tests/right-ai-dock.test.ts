...
describe('PDF right AI dock', () => {
  it('renders the persistent AI dock after the viewer content', () => {
    const source = read('../src/renderer/App.tsx')
    expect(source.indexOf('className={`pdf-scroll')).toBeLessThan(
      source.indexOf('<div className={`ai-dock${aiCollapsed'),
    )
