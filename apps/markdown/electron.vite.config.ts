...
export default defineConfig({
  // @wiswork/i18n and @wiswork/electron-utils ship as TS source — must be bundled
  main: {
    plugins: [externalizeDepsPlugin({ exclude: ['@wiswork/i18n', '@wiswork/electron-utils'] })],
  },
  preload: {
    plugins: [externalizeDepsPlugin({ exclude: ['@wiswork/i18n'] })],
  },
  renderer: {
    plugins: [react()],
