...
describe('buildLazyCellTest', () => {
  ...
  it('rejects an excessively long query without scanning', () => {
    expect(buildLazyCellTest(query({ findString: 'x'.repeat(4_097) }))).toBeNull()
  })
})
...
describe('collectJournalMatches', () => {
  ...
  it('bounds journal matches before allocating the full result set', () => {
    const entries = new Map(
      Array.from({ length: 20 }, (_, row) => [
        `${row}:0`,
        { row: row + 20, column: 0, value: 'needle', hasValue: true },
      ]),
    )
    const lazyState = state({
      journalCells: new Map([['s1', entries]]) as LazyWorkbookState['editJournal']['cells'],
    })
    expect(collectJournalMatches(lazyState, 's1', buildLazyCellTest(query())!, 3)).toHaveLength(3)
  })
})
      ...
      sheets: [
        {
          ...
          columnCount: overrides.columnCount ?? 8,
        },
      ],
    },
    expectedWorkbookId: 'workbook-1',
    generation: 1,
...
describe('installLazyFindBridge', () => {
  ...
  it('visits an out-of-window hit before a later in-window hit', async () => {
    const harness = facade(state({}))
    const inner = new CursorInnerModel([match('s1', 500, 1)])
    inner.onFocus = (row, column) => {
      harness.active.row = row
      harness.active.column = column
    }
    ...
    mockRead.mockResolvedValue(
      mapped([
        { row: 100, column: 1, value: 'early needle one' },
        { row: 200, column: 1, value: 'early needle two' },
      ]),
    )

    const model = (await harnessLookup(harness)(query()))[0]!
    await vi.waitFor(() => expect(model.getMatches()).toHaveLength(3))
    expect((model.moveToNextMatch({ loop: true }) as LazyCellMatch).range.range.startRow).toBe(100)
    expect((model.moveToNextMatch({ loop: true }) as LazyCellMatch).range.range.startRow).toBe(200)
    expect((model.moveToNextMatch({ loop: true }) as LazyCellMatch).range.range.startRow).toBe(500)
    bridge.dispose()
  })

  it('advances the logical extra cursor when navigation suppresses focus', async () => {
    const harness = facade(state({}))
    ...
    mockRead.mockResolvedValue(
      mapped([
        { row: 100, column: 1, value: 'needle one' },
        { row: 200, column: 1, value: 'needle two' },
      ]),
    )
    const model = (await harnessLookup(harness)(query()))[0]!
    await vi.waitFor(() => expect(model.getMatches()).toHaveLength(2))
    expect((model.moveToNextMatch({ noFocus: true }) as LazyCellMatch).range.range.startRow).toBe(
      100,
    )
    expect((model.moveToNextMatch({ noFocus: true }) as LazyCellMatch).range.range.startRow).toBe(
      200,
  ...
  it('drops a pending inner match removed by a filter refresh', async () => {
    const harness = facade(state({}))
    const innerMatches = [match('s1', 500, 1)]
    const inner = new CursorInnerModel(innerMatches)
    harness.providers.add({ find: vi.fn().mockResolvedValue([inner]), terminate: vi.fn() })
    const bridge = installLazyFindBridge(harness)
    mockRead.mockResolvedValue(
      mapped([
    ...
    await vi.waitFor(() => expect(model.getMatches()).toHaveLength(3))
    expect((model.moveToNextMatch({ loop: true }) as LazyCellMatch).range.range.startRow).toBe(100)
    innerMatches.splice(0)
    inner.matchesUpdate$.next([])
    expect((model.moveToNextMatch({ loop: true }) as LazyCellMatch).range.range.startRow).toBe(200)
    expect((model.moveToNextMatch({ loop: true }) as LazyCellMatch).range.range.startRow).toBe(100)
    bridge.dispose()
  })

  it('truncates explicitly before reading a row wider than the sidecar batch cap', async () => {
    const harness = facade(state({ rowCount: 1, columnCount: 20_000 }))
    const inner = new FakeInnerModel([])
    harness.providers.add({ find: vi.fn().mockResolvedValue([inner]), terminate: vi.fn() })
    const bridge = installLazyFindBridge(harness)
    await harnessLookup(harness)(query())
  ...
  it('binds every sidecar batch to abort and workbook identity guards', async () => {
    const lazyState = state({})
    const harness = facade(lazyState)
    const inner = new FakeInnerModel([])
    harness.providers.add({ find: vi.fn().mockResolvedValue([inner]), terminate: vi.fn() })
    const bridge = installLazyFindBridge(harness)
    let capturedOptions: Parameters<typeof readSheetRangeMapped>[4]
    mockRead.mockImplementation(async (_state, _sheetId, _range, _meta, options) => {
      capturedOptions = options
      harness.lazyWorkbookRef.current = null
      return mapped([{ row: 500, column: 3, value: 'stale needle' }])
    })
