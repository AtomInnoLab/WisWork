...
/** office/pdf formats extract text via @wiswork/file-parse; images skip text
...
/** Attachment text extraction via @wiswork/file-parse (docx/pdf/pptx/xlsx/plain text) */
...
export function registerSheetsIpc(): void {
   ...
   */
  ipcMain.handle('sheets:has-queued-workbook', (event) => {
    sessionFor(event)
    return hasQueuedWorkbook()
  })
...
async function prepareWorkbookForOpen(
  ...
  const directory = join(app.getPath('temp'), 'wiswork-imports', randomUUID())
