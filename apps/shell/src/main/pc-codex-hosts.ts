...
interface IpcMainLike {
  handle(channel: string, handler: (event: { sender: PcOwner }, ...args: unknown[]) => unknown): void
}
...
const exactObject = (value: unknown, keys: readonly string[]): value is globalThis.Record<string, unknown> => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  if (Object.getPrototypeOf(value) !== Object.prototype || Object.getOwnPropertySymbols(value).length)
    return false
  const descriptors = Object.getOwnPropertyDescriptors(value)
  return Object.keys(descriptors).length === keys.length && keys.every((key) => 'value' in (descriptors[key] ?? {}))
}
const detachedExecution = (value: unknown): ToolExecution => {
  ...
  if (Object.getOwnPropertySymbols(value).length || Object.keys(descriptors).some((key) => !allowed.has(key)) ||
    Object.values(descriptors).some((descriptor) => !('value' in descriptor))) throw new Error('enhanced_invalid_request')
  ...
  if (typeof output !== 'string' || Buffer.byteLength(output) > 1_000_000 ||
    typeof summary !== 'string' || Buffer.byteLength(summary) > 4_096 ||
    (isError !== undefined && typeof isError !== 'boolean') ||
    (mutated !== undefined && typeof mutated !== 'boolean') ||
    (stopToolBatch !== undefined && typeof stopToolBatch !== 'boolean')) throw new Error('enhanced_invalid_request')
  return Object.freeze({ output, summary, ...(isError === undefined ? {} : { isError }),
...
export function registerPcCodexHosts(options: {
  ...
  const send = (record: HostRecord, channel: string, value: unknown) => {
    ...
    try { record.owner.send(channel, value) } catch {}
  }
  const dispatch = (record: HostRecord, call: AgentToolCall, claim?: object): Promise<ToolExecution> =>
    new Promise((resolve, reject) => {
      if (record.closed || record.pending.has(call.id)) return reject(new Error('tool_session_closed'))
      record.pending.set(call.id, { call, resolve, reject: () => reject(new Error('tool_execution_failed')), claim })
      send(record, PC_HOST_CODEX_CHANNELS.event, { type: 'tool-start', call })
      send(record, PC_HOST_CODEX_CHANNELS.toolCall, {
        documentId: record.documentId, generation: record.generation, call,
      })
    })
  ...
  const handleEngineEvent = (record: HostRecord, event: import('./codex-runtime').CodexRuntimeEngineEvent) => {
    if (event.type === 'text') {
    ...
    if (event.status === 'failed')
      ...
      send(record, PC_HOST_CODEX_CHANNELS.event, { type: 'done', result: {
        text: record.text, cancelled: event.status === 'cancelled', turnLimit: false,
      } })
    record.text = ''
  }
  ...
  options.ipcMain.handle(PC_HOST_CODEX_CHANNELS.status, (event, ...args) => {
    ...
    return { activeAgentRuntime: options.runtime.configuredAgentRuntime,
      documentId: record?.documentId ?? null }
  })

  options.ipcMain.handle(PC_HOST_CODEX_CHANNELS.register, async (event, value) => {
    const host = trusted(event.sender)
    if (!exactObject(value, ['host', 'documentId', 'generation', 'systemPrompt', 'tools', 'mutatingTools']))
      throw new Error('enhanced_invalid_request')
    const input = value as unknown as PcHostRegistration
    if (input.host !== host) throw new Error('enhanced_untrusted_request')
    if (!ID.test(input.documentId) || !Number.isSafeInteger(input.generation) || input.generation < 0 ||
      typeof input.systemPrompt !== 'string' || Buffer.byteLength(input.systemPrompt) > 256_000 ||
      !Array.isArray(input.tools) || !Array.isArray(input.mutatingTools))
      throw new Error('enhanced_invalid_request')
    if (records.has(event.sender) || byDocument.has(input.documentId)) throw new Error('enhanced_document_exists')
    const authority = createShellEnhancedPolicyAuthority(() => input.generation)
    const grant = authority.issue({ generation: input.generation, host, policy: options.policy,
      capabilities: ['semantic-read', 'transaction-proposal'] })
    const mutating = new Set(input.mutatingTools)
    const manifest = createDocumentToolManifest({ policyGrant: grant,
      consumePolicyGrant: (candidate) => authority.consume(candidate as never), tools: input.tools,
      policy: Object.fromEntries(input.tools.map((tool) => [tool.name, mutating.has(tool.name) ? 'mutate' : 'read'])) })
    const inertSkill: AgentSkill = { id: `enhanced-${host}`, systemPrompt: '', tools: [],
      executeTool: async () => ({ output: 'denied', summary: 'denied', isError: true }) }
    const harness = createAgentHarness({ skill: inertSkill, maxTurns: 1,
      transport: { stream: (_request, callbacks) => { callbacks.onError('transport_disabled'); return { cancel() {} } } } })
    let record!: HostRecord
    const session = createDocumentToolSession({
      identity: { ownerId: `renderer-${event.sender.id}`, host, documentId: input.documentId,
        sessionId: randomBytes(16).toString('hex'), generation: input.generation },
      manifest, isOpen: () => !record.closed && !event.sender.isDestroyed(),
      executeRead: (call) => dispatch(record, call),
      suspendMutation: (result) => {
        const suspension = harness.suspendToolExecution(result)
        queueMicrotask(() => {
          const claimed = session.mutationAuthority.claimNext()
          if (!claimed || record.closed) return
          void dispatch(record, claimed.request.call, claimed.claim).catch(() =>
            session.mutationAuthority.reject(claimed.claim, 'tool_execution_failed'))
        })
        return suspension
      },
      ownsSuspension: (value) => harness.ownsToolExecutionSuspension(value),
    })
    const close = options.runtime.registerDocument({ owner: event.sender, documentId: input.documentId,
      host, generation: input.generation, toolSession: session, instructions: input.systemPrompt,
      onEvent: (engineEvent) => handleEngineEvent(record, engineEvent) })
    record = { owner: event.sender, documentId: input.documentId, generation: input.generation,
      closeRuntime: close.close, harness, pending: new Map(), session, text: '', closed: false }
    records.set(event.sender, record)
    byDocument.set(input.documentId, record)
  })

  options.ipcMain.handle(PC_HOST_CODEX_CHANNELS.unregister, async (event, documentId, generation) => {
    trusted(event.sender)
    const record = records.get(event.sender)
  ...
  options.ipcMain.handle(PC_HOST_CODEX_CHANNELS.toolResult, (event, value) => {
    ...
    if (!record || record.documentId !== result.documentId || record.generation !== result.generation || !pending)
      throw new Error('enhanced_untrusted_request')
    record.pending.delete(result.callId)
    send(record, PC_HOST_CODEX_CHANNELS.event, { type: 'tool-executed', event: { call: pending.call, execution } })
  ...
  return Object.freeze({
    ...
    closeOwner: async (owner: CodexOwner) => { const record = records.get(owner as PcOwner); if (record) await closeRecord(record) },
    close: async () => { for (const record of [...records.values()]) await closeRecord(record) },
  })
}
