...
describe('initial state', () => {
  it('starts with only the non-closable, active Home tab', () => {
    expect(manager.list()).toEqual([
      { id: 'home', kind: 'home', title: 'WisWork', closable: false, active: true },
    ])
  })
})
...
describe('opening tabs', () => {
  ...
  it('uses module default titles for pathless tabs', () => {
    ...
    expect(manager.list().map((t) => t.title)).toEqual(['WisWork', 'AI Sheets', 'AI Slides'])
  })
...
describe('closing tabs', () => {
  ...
  it('quiesces the document runtime before checking or prompting for dirty state', async () => {
    let release!: () => void
    ...
    const prepareDocumentClose = vi.fn(async () => {
      await runtimeClosed
    ...
    expect(prepareDocumentClose).toHaveBeenCalledWith({
      documentId: id,
      kind: 'sheets',
      webContents: view.webContents,
    })
    expect(requestSheetsClose).not.toHaveBeenCalled()
    expect(shellWindow.contentView.removeChildView).not.toHaveBeenCalled()
    release()
    await closing
    expect(requestSheetsClose).toHaveBeenCalledWith(view.webContents, shellWindow)
    expect(commit).toHaveBeenCalledOnce()
    expect(rollback).not.toHaveBeenCalled()
    expect(shellWindow.contentView.removeChildView).toHaveBeenCalledWith(view)
    expect(finalize).toHaveBeenCalledOnce()
    expect(shellWindow.contentView.removeChildView.mock.invocationCallOrder[0]).toBeLessThan(
      finalize.mock.invocationCallOrder[0]!,
    )
  })

  it('resumes a quiesced document session when the user cancels its close prompt', async () => {
    sheetsPendingEditCount.mockImplementation(() => 1)
    requestSheetsClose.mockResolvedValue(false)
    const commit = vi.fn(async () => undefined)
    const rollback = vi.fn()
    const finalize = vi.fn()
    manager = new TabManager(
      ...
      (kind) => applyMenuFor(kind),
      undefined,
      async () => ({ commit, rollback, finalize }),
    )
    const id = manager.openSheetsTab()
  ...
  it('finalizes exactly once when fallback activation throws after identity removal', async () => {
    const finalize = vi.fn()
    manager = new TabManager(
      shellWindow as never,
      () => onChanged(),
      (kind) => applyMenuFor(kind),
      undefined,
      async () => ({ commit: async () => undefined, rollback: vi.fn(), finalize }),
    )
    const id = manager.openSheetsTab()
    const view = lastCreatedView(createSheetsView)
    applyMenuFor.mockImplementation(() => {
      throw new Error('menu failure with untrusted detail')
    })

    await expect(manager.closeTab(id)).resolves.toBeUndefined()

    expect(manager.ownsDocument(view.webContents.id, id)).toBe(false)
    expect(finalize).toHaveBeenCalledOnce()
  })

  it('finalizes exactly once when Electron view removal throws after identity removal', async () => {
    const finalize = vi.fn()
    manager = new TabManager(
      shellWindow as never,
      () => onChanged(),
    ...
    const id = manager.openSheetsTab()
    const view = lastCreatedView(createSheetsView)
    shellWindow.contentView.removeChildView.mockImplementation(() => {
      throw new Error('electron detach failure')
    })
  ...
  it.each(['renderer-close', 'renderer-teardown'] as const)(
    'finalizes exactly once when %s throws after identity removal',
    async (failure) => {
      const finalize = vi.fn()
      manager = new TabManager(
        shellWindow as never,
      ...
      const id = failure === 'renderer-close' ? manager.openSheetsTab() : manager.openDocsTab()
      const view = lastCreatedView(failure === 'renderer-close' ? createSheetsView : createDocsView)
      if (failure === 'renderer-close') {
        view.webContents.close.mockImplementation(() => {
          throw new Error('renderer close failure')
        })
      } else {
        teardownDocsRenderer.mockImplementation(() => {
          throw new Error('renderer teardown failure')
        })
      }
  ...
  it('maps a renderer only to its authoritative tab document identity', () => {
    const id = manager.openSheetsTab()
    const view = lastCreatedView(createSheetsView)

    expect(manager.documentIdForWebContents(view.webContents.id)).toBe(id)
    expect(manager.ownsDocument(view.webContents.id, id)).toBe(true)
    expect(manager.ownsDocument(view.webContents.id, 'other')).toBe(false)
  })
