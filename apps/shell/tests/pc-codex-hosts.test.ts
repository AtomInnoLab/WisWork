...
describe('PC Codex host registrar', () => {
  it('binds registration to the trusted host and dispatches reads and mutations to its owner', async () => {
    ...
    const owner = { id: 7, isDestroyed: () => false, send: (c: string, v: unknown) => sent.push([c, v]) }
    let registered: any
    const engine = {
      ...
      startTurn: vi.fn(async () => undefined), cancelTurn: vi.fn(async () => undefined),
      closeDocument: vi.fn(async () => undefined), close: vi.fn(async () => undefined),
    }
    const policy = { globalEnabled: true, rawOfficeEnabled: false,
      hosts: Object.fromEntries(ENHANCED_HOSTS.map((host) => [host, true])) as any }
    const runtime = new ShellCodexRuntime({ activeAgentRuntime: 'enhanced', policy,
      isSignedIn: async () => true, resolveExecutable: async () => '/private/codex',
      bootstrap: { start: async () => engine } })
    await runtime.initialize()
    const registrar = registerPcCodexHosts({
      ipcMain: { handle: (channel, handler) => handlers.set(channel, handler) }, runtime, policy,
      hostForOwner: (candidate) => candidate === owner ? 'docs' : null,
    })
    await handlers.get(PC_HOST_CODEX_CHANNELS.register)!({ sender: owner }, {
      host: 'docs', documentId: 'doc-1', generation: 3, systemPrompt: 'docs rules',
      tools: [
    ...
    const read = registered.session.callTool(registered.session.credentials,
      { id: 'r1', name: 'read_blocks', input: {} }) as Promise<unknown>
    expect(sent.at(-1)?.[0]).toBe(PC_HOST_CODEX_CHANNELS.toolCall)
    await handlers.get(PC_HOST_CODEX_CHANNELS.toolResult)!({ sender: owner }, {
      documentId: 'doc-1', generation: 3, callId: 'r1',
  ...
  it('rejects host substitution before registering a document', async () => {
    ...
    registerPcCodexHosts({ ipcMain: { handle: (c, h) => handlers.set(c, h) },
      runtime: { registerDocument: vi.fn() } as any,
      policy: { globalEnabled: true, rawOfficeEnabled: false, hosts: { docs: true } } as any,
      hostForOwner: () => 'docs' })
    await expect(handlers.get(PC_HOST_CODEX_CHANNELS.register)!({ sender: owner }, {
      host: 'slides', documentId: 'doc', generation: 0, systemPrompt: '', tools: [], mutatingTools: [],
