...
import { AgentLoop, composeSkills, type AgentImage, type ToolDisplay } from '@wiswork/agent-core'
...
export function AiPanel({
  ...
  if (!loopRef.current) {
    ...
    // - All requests use the fixed WisWork settings resolved by the main process.
    // errKind marks failure categories that should not be retried (timeout/empty output/user stop).
    type LlmResult = {
      ok: boolean
      text?: string
      error?: string
    ...
    const access: DeckAccess = {
      ...
      generateStyleSkill: async (a) => {
        ...
        const r = await runLlmOnce(sys, userMsg, 90000, a.signal)
        return r.ok && r.text
