...
test('requires an explicit proposal apply, blocks replay, and supports undo', async () => {
  ...
  try {
    ...
    await expect(review.locator('.diff-line-remove code')).toContainText('WisWork')
    await expect(review.locator('.diff-line-add code')).toContainText('AI-confirmed WisWork')
    const selection = review.getByRole('checkbox', { name: 'main.tex' })
    await expect(selection).toBeChecked()
    await selection.uncheck()
    await expect(review.getByRole('button', { name: 'Verify selected' })).toBeDisabled()
    await selection.check()
    await review.getByRole('button', { name: 'Apply verified changes' }).click()
