...
if (!firstPid || !restartedPid || firstPid === restartedPid || sessionRoots[0] === sessionRoots[1])
...
if (
  !shouldStartEnhancedRuntime({ requested: 'enhanced', active: 'enhanced' }, enabled, 'office-word')
)
...
if ((await manager.status()).state !== 'missing') throw new Error('native_lifecycle_remove_failed')
...
if ((await fallback.status()).state !== 'ready') throw new Error('native_lifecycle_fallback_failed')
