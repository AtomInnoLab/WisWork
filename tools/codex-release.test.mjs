...
test('native lifecycle pairs private session creation with bounded cleanup', () => {
  ...
  assert.match(lifecycle, /maxRetries:\s*300/)
  assert.match(lifecycle, /retryDelay:\s*100/)
  assert.match(lifecycle, /serverChildren\.push\(child\)/)
  assert.doesNotMatch(lifecycle, /args\[0\] === ['"]app-server['"]/)
  assert.match(lifecycle, /standardRuntime\.shutdown\(\)/)
  ...
  assert.match(processManager, /maxRetries:\s*300/)
